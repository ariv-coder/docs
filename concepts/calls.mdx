...
VoiceBook AI can handle both inbound and outbound phone calls. This guide covers how calls work and how to manage them.

## Call Types

### Inbound Calls

When someone calls your VoiceBook AI phone number:
...
```bash
curl https://api.voicebook.ai/v1/calls/call_abc123 \
  -H "Authorization: Bearer vb_live_YOUR_API_KEY"
```
...
```bash
curl https://api.voicebook.ai/v1/campaigns \
  -X POST \
  -H "Authorization: Bearer vb_live_YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
