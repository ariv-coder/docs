...
| `call.completed` | Call ended successfully (includes satisfaction score) |
...
| Event | Description |
|-------|-------------|
| `appointment.booked` | New appointment booked by AI agent |
...
| `appointment.no_show` | Customer didn't show up |
| `appointment.reminder` | Reminder was sent |

<Info>
  These events are Zapier-compatible. Use them with Zapier's webhook trigger or any automation platform.
</Info>
...
Always verify webhook signatures to ensure they came from Ringyo AI. Webhooks are signed using **HMAC-SHA256**.
...
- `v1` — HMAC-SHA256 hash of `{timestamp}.{payload}`
...
func VerifyWebhook(payload, signature, secret string) error {
    parts := strings.Split(signature, ",")
    timestamp := strings.TrimPrefix(parts[0], "t=")
    hash := strings.TrimPrefix(parts[1], "v1=")
...
Monitor webhook delivery in the **Developers → Webhooks** dashboard:

- **Recent deliveries** — See the last 100 webhook attempts
- **Status** — Success (2xx), Failed, or Pending retry
- **Response time** — How long your server took to respond
- **Payload** — View the exact JSON sent
- **Response** — See your server's response (for debugging)

<Tip>
  Use delivery logs to debug integration issues. Failed webhooks show the error response from your server.
</Tip>
